...
#define THROW_IF_CONSOLE_ERROR(res) if(!res) throw XError(__LINE__,__FILE__)
	...
	string Win32ThinWrapper::GetTitle(DWORD nSize)
	{
		...
		delete lpConsoleTitle;
		string title = lpConsoleTitle;
		return title;
	}
	...
	{
		DWORD lpNumberOfCharsWritten;
		THROW_IF_CONSOLE_ERROR(::WriteConsoleOutputCharacterA(hOutput_, lpCharacter, nLength, dwWriteCoord, &lpNumberOfCharsWritten));
		return lpNumberOfCharsWritten;
	}
}
